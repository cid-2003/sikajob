...
<body>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8 mx-auto">
                    ...
                    <div class="bg-mode p-4">
                        <h1 class="h4 mb-4">Candidatures</h1>
                        <hr class="my-4">
                        {{--Affichage des notif de candidature chez les recruteurs--}}
                        ...
                        <div class="card bg-transparent border-0">
                            <div class="row g-3 align-items-center border-bottom pb-3">
                                ...
                                <div class="row g-3 align-items-center notification-item bg-light bg-opacity-50 p-3 rounded-3 mb-3">
                                    <div class="col-3 col-md-2">
                                        <img class="rounded w-100" src="{{ $candidature->user->photo ? asset('storage/' . $candidature->user->photo) : asset('assets/images/avatar/placeholder.jpg') }}" alt="">
                                    </div>
                                    <div class="col-9 col-md-10">
                                        <!-- Badge pour le statut -->
                                        <span class="badge bg-info bg-opacity-10 text-info mb-1 fw-bold">{{ ucfirst($candidature->status) }}</span>
                                        
                                        <!-- Nom du candidat et poste -->
                                        <h5 class="mb-1">
                                            @php
                                            preg_match('/^(.*?) pour| à| sur| dans/', $candidature->publication->content, $matches);
                                            $titreAccrocheur = $matches[1] ?? Str::limit($candidature->publication->content, 10);
                                           @endphp

                                            <a href="{{ route('notif-detail', ['id' => $candidature->id]) }}" class="btn-link stretched-link text-reset fw-bold">
                                                Vous avez reçu une nouvelle candidature de {{ $candidature->user->name }} {{ $candidature->user->prenom }} 
                                                – pour "{{ $titreAccrocheur }}"
                                             </a>
                                        </h5>
                                        
                                        <!-- Informations supplémentaires -->
                                        <div class="d-flex flex-wrap text-muted small">
                                            <span class="me-3"><i class="fa fa-envelope me-1"></i> {{ $candidature->user->email }}</span>
                                            <span class="me-3"><i class="fa fa-phone me-1"></i> {{ $candidature->user->settings->number ?? 'Non renseigné' }}</span>
                                            <span><i class="fa fa-clock me-1"></i> {{ $candidature->created_at->diffForHumans() }}</span>
                                        </div>
                                        
                                        <!-- Extrait de la lettre de motivation -->
                                        <p class="mt-2 mb-0 text-truncate d-none d-md-block">
                                            "{{ Str::limit($candidature->letter) }}"
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        {{--Affichage des notif de candidature pour les candidats--}}
                        @foreach ($candidatures as $candidature)
                        <div class="card bg-transparent border-0">
                            <div class="row g-3 align-items-center border-bottom pb-3">
                                <div class="row g-3 align-items-center notification-item bg-light bg-opacity-50 p-3 rounded-3 mb-3">
                                    ...
                                    <small class="text-muted d-block mb-2">
                                        ...
                                        @php
                                            preg_match('/^(.*?) pour| à| sur| dans/', $candidature->publication->content, $matches);
                                            $titreAccrocheur = $matches[1] ?? Str::limit($candidature->publication->content, 25);
                                           @endphp

                                            <h5 class="text-reset fw-bold">
                                                Vous avez postulé à {{ $titreAccrocheur }}
                                                – de {{ $candidature->publication->user->name }} {{ $candidature->publication->user->prenom }}
                                            </h5>
                                             Postulé le {{ $candidature->created_at->format('d/m/Y H:i') }}
                                    </small>
                                    ...
                                    @if ($candidature->status === 'acceptée')
                                        <span class="badge bg-success"><h5>{{ $candidature->publication->user->name }} a été acceptée votre candidature. Félicitations!</h5></span>
                                    @elseif ($candidature->status === 'refusée')
                                        <span class="badge bg-danger"><h5>{{ $candidature->publication->user->name }} a été refusée votre candidature.</h5></span>
                                    @endif
                                    </div>
                                    ...
                                    @if ($candidature->status !== 'acceptée' && $candidature->status !== 'refusée')
                                        <button class="btn btn-sm btn-outline-primary"  
                                            ...
                                            onclick="loadEditModal({{ $candidature->id }})">
                                            <i class="fas fa-pencil-alt me-1"></i> Modifier
                                        </button>
                                    @endif
                                
                                        <!-- Bouton Supprimer -->
                                        <form method="POST" action="{{ route('candidature.supprime', $candidature->id) }}">
                                            @csrf 
                                            @method('DELETE')
                                            <button type="submit" 
                        ...
                        @endforeach 
                            @if(isset($candidatures) && $candidatures->count() > 0)
                                 ...
                                 @endforeach
                            @else
                                  <p>Vous avez postuler à aucune offre ppour le moment.</p>
                            @endif                        
                        
                        <!-- Modal de Modification -->
                        <form id="editCandidatureForm" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Modifier ma candidature</h5>
                                    ...
                                    <div class="modal-body">
                                        <!-- Nouveau CV -->
                                        <div class="mb-3">
                                            <label class="form-label">Changer le CV :</label>
                                            <input type="file" class="form-control" form="editCandidatureForm" id="cv" name="cv" accept=".pdf,.doc,.docx">
                                        </div>
                        
                                        <!-- Lettre de motivation -->
                                        <div class="mb-3">
                                            <label class="form-label">Lettre de motivation :</label>
                                            <textarea class="form-control" id="letter" rows="6" name="letter" form="editCandidatureForm"></textarea>
                                        </div>
                                    </div>
        ...
        <div class="row">
          <div class="col-md-6">
            ...
            <ul class="nav justify-content-center justify-content-md-start lh-1">
              <li class="nav-item">
                <a class="nav-link" target="_blank" href="{{ route('index') }}">Accueil </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="{{ route('my-profile') }}">Profil</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="{{ route('settings') }}">Paramètre</a>
              </li>
          ...
          <div class="col-md-6">
            <!-- Copyright START -->
            <p class="text-center text-md-end mb-0">©2025 <a class="text-body" target="_blank" href=""> SikaJob </a>All rights reserved.</p>
            <!-- Copyright END -->
          </div>
        </div>
      </div>
    ...
    fetch(`/candidature/${candidatureId}/edit`)
        .then(response => response.json())
        .then(data => {
            // Affiche ancienne lettre
            document.getElementById('letter').value = data.letter;

            // Stocke l'ID pour la requête de mise à jour
            document.getElementById('saveChanges').setAttribute('data-id', candidatureId);
        });
}

document.getElementById('saveChanges').addEventListener('click', function () {
    const candidatureId = this.getAttribute('data-id');
    const form = document.getElementById('editCandidatureForm');
    
    form.setAttribute('action', `/candidature/${candidatureId}/update`);
    form.submit();
});
